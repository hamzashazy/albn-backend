...
const getBatches = async (req, res) => {
  try {
    const batches = await Batch.find().sort({ createdAt: -1 }).populate('campus', 'name');
...
const getBatchesByCampus = async (req, res, next) => {
  try {
    ...
    const batches = await Batch.find({ campus: campusId })
...
const getActiveBatches = async (req, res) => {
  try {
    const batches = await Batch.find({ isDeleted: false }).populate('campus', 'name');
    res.status(200).json(batches);
  } catch (error) {
